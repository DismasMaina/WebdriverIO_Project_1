...
async function findAndClickTicket(ticket) {
  ...
  try {
    ...
    const ticketRows = await $$('table tbody tr:not(.ant-table-measure-row):not([class*="placeholder"])');
    ...
    if (!foundRow) {
      ...
      throw new Error(`Ticket ${ticket} not found in queue (checked ${ticketRows.length} data rows)`);
    }
...
async function serveTicket(ticket) {
  try {
    ...
    await browser.waitUntil(
      async () => {
        const searchField = await $('input[placeholder*="Search by Ticket Number or Patient Name"]').catch(() => null);
